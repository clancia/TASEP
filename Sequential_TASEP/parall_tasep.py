...
def seqHIT(b, p=1.0, e=0.0):
    """
    Hitting times of the Sequential TASEP
    """
    print "Computes hitting time from a random sigma configuration to another random tau configuration"
    n=2*b
    sigma = np.concatenate((np.ones(b),np.zeros(b)), axis=0)
    tau = np.copy(sigma)
    while (sigma == tau).all():
        npr.shuffle(sigma)
        npr.shuffle(tau)   
    t=0
    while not (sigma==tau).all():  #compare every value of the vectors
        i = npr.randint(0,n)
        if sigma[i%n] and not sigma[(i+1)%n] and npr.random() < p:
            sigma[i%n] = 0.0 
            sigma[(i+1)%n] = 1.0
        t += 1
    return t

def seqTASEP(b, p=1.0, e=0.0):
    """
    Coalescence times of the Sequential TASEP
    """
    n=2*b
    sx = np.concatenate((np.ones(b),np.zeros(b)), axis=0)
    while sx[0] or not sx[-1]:
        npr.shuffle(sx)
    dx = np.copy(sx)
    dx[0] = 1.0             
    dx[-1] = 0.0            
    t=0
    while not (sx==dx).all():  #compare every value of the vectors
        i=npr.randint(0,2*b)
        if sx[i%n] and not sx[(i+1)%n] and npr.random() < p:
            sx[i%n] = 0.0 
            sx[(i+1)%n] = 1.0
        if dx[i%n] and not dx[(i+1)%n] and npr.random() < p:
            dx[i%n] = 0.0 
            dx[(i+1)%n] = 1.0
        t += 1
    return t
